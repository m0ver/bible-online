...
{  
	if(typeof arguments[0] == 'string'){  
		...
		results = arguments[0].match(/^ *(\d{4})-(\d{1,2})-(\d{1,2}) +(\d{1,2}):(\d{1,2}):(\d{1,2}) *$/);
		if(results && results.length>6)
		return new Date(parseInt(results[1]),parseInt(results[2]) -1,parseInt(results[3]),parseInt(results[4]),parseInt(results[5]),parseInt(results[6]));

		results = arguments[0].match(/^ *(\d{4})-(\d{1,2})-(\d{1,2}) +(\d{1,2}):(\d{1,2}):(\d{1,2})\.(\d{1,9}) *$/);
...
struct.loading=function(){
	...
	this.element=typeof(arguments[0])!=undefined?arguments[0]:document.body;
	this.start=function()
	{
    	if(!this.icon)
    	{
    	...
    	else this.icon.style.display='block';
...
struct.fixed = function(element ,top ,left){
	...
	this.addCSSRule = function(key ,value){
	   var css = document.styleSheets[document.styleSheets.length-1];
		css.cssRules ? (css.insertRule(key +"{"+ value +"}", css.cssRules.length)) :
